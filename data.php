<?php
        ...
        $sql2 = "SELECT * FROM messages WHERE (incoming_msg_id = '{$row['unique_id']}' OR outgoing_msg_id = '{$row['unique_id']}')
                		          AND (outgoing_msg_id = '{$membershipno}' OR incoming_msg_id = '{$membershipno}')
        ...
        if(isset($row2['outgoing_msg_id'])){
            ($membershipno == $row2['outgoing_msg_id']) ? $you = "You: " : $you = "";
        }else{
            $you = "";
        }
        ($row['status'] == "Offline now") ? $offline = "offline" : $offline = "";
        ($membershipno == $row['unique_id']) ? $hid_me = "hide" : $hid_me = "";
